...
//BELOW CODE SORTS FETCHED ITEMS
	if(isset($_POST['sort']))
	{
				...
				if ($sort_criteria=='az') {
					...
					$sql= mysqli_query($server," SELECT * FROM  `product_details`WHERE sub_name='$sub_name' ORDER BY  `product_details`.`name` DESC LIMIT $start_from , 10");
				
				} else if($sort_criteria=='cheapest'){
				$sql= mysqli_query($server,"SELECT * FROM  `product_details` WHERE sub_name='$sub_name' ORDER BY  `product_details`.`current_price` ASC LIMIT $start_from , 10 ");

				} else if ($sort_criteria=='expensive'){
				$sql= mysqli_query($server,"SELECT * FROM  `product_details` WHERE sub_name='$sub_name' ORDER BY  `product_details`.`current_price` DESC LIMIT $start_from , 10 ");
				
				} else {
				$sql= mysqli_query($server,"SELECT * FROM product_details WHERE sub_name='$sub_name' ORDER BY  `product_details`.`product_id` DESC LIMIT $start_from , 10  ");
				}
		...
		$sql= mysqli_query($server,"SELECT * FROM product_details WHERE sub_name='$sub_name'
		 ORDER BY  `product_details`.`product_id` ASC LIMIT $start_from , 10   ");			
	}
...
//pagination 1
 ...
 while($colms=mysqli_fetch_array($sql, MYSQLI_NUM))
 {
...
//pagination 2
		echo"	<div class='blog-pagenat'>
				<ul>";	  
	$sql =mysqli_query($server, "SELECT count(product_id) AS total FROM product_details WHERE sub_name='$sub_name' ORDER BY  `product_details`.`product_id` ASC ");
	...
	$total_pages = ceil($total_records/10);
		...
		if ($pageno>1) {
			echo "<li><a href='e_prods_sub.php?sub=".$sub_name."&pageno=".($pageno-1)."'>&lsaquo;</a></li>";
	
		}
	for ($i=1; $i <=$total_pages ; $i++) 
	{ 
			echo "<li><a href='e_prods_sub.php?sub=$sub_name&pageno=$i'>$i</a></li>";
	}
		if ($pageno < $total_pages )
		 {

			echo "<li><a href='e_prods_sub.php?sub=$sub_name&pageno=".($pageno+1)."'>&rsaquo;</a></li>";
