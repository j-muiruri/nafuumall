...
$count = mysqli_query($server, "SELECT COUNT(*) FROM product_details WHERE cat_name='$cat_name' ");
...
//BELOW CODE SORTS FETCHED ITEMS
	if(isset($_POST['sort']))
	{
				...
				if ($sort_criteria=='az') {
					$ress= mysqli_query($server," SELECT * FROM  `product_details`WHERE cat_name='$cat_name' ORDER BY  `product_details`.`name` ASC LIMIT $start_from , 10");

				} else if($sort_criteria=='za') {
					$ress= mysqli_query($server," SELECT * FROM  `product_details`WHERE cat_name='$cat_name' ORDER BY  `product_details`.`name` DESC LIMIT $start_from , 10");
				
				} else if($sort_criteria=='cheapest'){
				$ress= mysqli_query($server,"SELECT * FROM  `product_details` WHERE cat_name='$cat_name' ORDER BY  `product_details`.`current_price` ASC LIMIT $start_from , 10 ");

				} else if ($sort_criteria=='expensive'){
				$ress= mysqli_query($server,"SELECT * FROM  `product_details` WHERE cat_name='$cat_name' ORDER BY  `product_details`.`current_price` DESC LIMIT $start_from , 10 ");
				
				} else {
				$ress= mysqli_query($server,"SELECT * FROM product_details WHERE cat_name='$cat_name' ORDER BY  `product_details`.`product_id` DESC LIMIT $start_from , 10  ");
				}
				
		
	}
	else{
		$ress= mysqli_query($server,"SELECT * FROM product_details WHERE cat_name='$cat_name'
...
//pagination 2
		echo"	<div class='blog-pagenat'>
				<ul>";	  
	$sql =mysqli_query($server, "SELECT count(product_id) AS total FROM product_details WHERE cat_name='$cat_name' ORDER BY  `product_details`.`product_id` ASC ");
	...
	$total_pages = ceil($total_records/10);
		...
		if ($pageno>1) {
			echo "<li><a href='e_prods_cat.php?cat=".$cat_name."&pageno=".($pageno-1)."'>&lsaquo;</a></li>";
		//	echo "<li><a class='frist' href='mainsearch.php?pageno=".($pageno-1)."'>Prev</a></li>";
		}
	for ($i=1; $i <=$total_pages ; $i++) 
	{ 
			echo "<li><a href='e_prods_cat.php?cat=".$cat_name."&pageno=".$i."'>$i</a></li>";
	}
		if ($pageno < $total_pages )
		 {

			echo "<li><a href='e_prods_cat.php?cat=".$cat_name."&pageno=".($pageno+1)."'>&rsaquo;</a></li>";
