...
if(!isset($_SESSION['sess_array']['0']) || $_SESSION['sess_array']['0'] == 'no_email' )
													
{
	$urim = $_SERVER['SCRIPT_NAME'];
	$uri = explode("/", $urim);

	if ($uri[1] !== 'sp_login.php' ) {
...
//account menu
$select =  mysqli_query($server, "SELECT * FROM `seller_details` WHERE email='$myemail'");
        ...
        <li role='presentation'><a role='menuitem' tabindex='-1' href='#'>Account info <span class'icon-use'></span></a></li>
          ...
          <li role='presentation'><a role='menuitem' tabindex='-1' onclick='logoutPrompt()' href='sp_login.php?log_out'>Log out</a></li>
        </ul>
		</div>";
		}
//account menu
...
$res= mysqli_query($server, "SELECT * FROM seller_details WHERE email='$email' AND password='$password'");
//check rows returned
...
if($_SESSION['sess_array']['0'] == 'no_email')
{
	...
	{
	 echo'<li class="">
	 <a href="sp_login.php?log_out" onclick="logoutPrompt()" style="padding-right:0" >
